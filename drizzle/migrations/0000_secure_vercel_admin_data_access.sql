...
DECLARE
  ...
  item_id uuid;
BEGIN
  IF encode(digest(coalesce(_admin_password, '') || ':' || coalesce(_session_secret, ''), 'sha256'), 'hex') <> 'e1b28046e0421218630ab552ef42357107618a5d1d073bfae9fd08327aa283a5' THEN
  ...
  IF _action = 'events' THEN
    SELECT coalesce(jsonb_agg(to_jsonb(e) ORDER BY e.created_at DESC), '[]'::jsonb)
      INTO result
      FROM (SELECT tipo, nome, created_at FROM public.analytics_events
            WHERE created_at >= now() - make_interval(days => safe_days)
            ORDER BY created_at DESC LIMIT 50000) e;
    RETURN result;
